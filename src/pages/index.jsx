...
export default function Home() {
  ...
  const getBlogPosts = useCallback(
    (amount) => {
      fetch(
        `https://frontend-case-api.sbdev.nl/api/posts?page=1&perPage=${amount}&sortBy=created_at&sortDirection=desc&searchPhrase=`,
        {
          method: "GET",
          redirect: "follow",
          headers: {
      ...
      <main className="grid grid-cols-12 gap-x-8 max-w-6xl w-11/12 m-auto">
        ...
        <section className="flex flex-col justify-between bg-white col-span-7 p-6 shadow-[0_8px_24px_rgba(149,157,165,0.2)]">
          <div className="grid grid-cols-2 gap-6">
            {received &&
              receivedBlogs.map((blog, key) => (
                <BlogPost blogPost={blog} key={key} />
              ))}
          </div>
          <button
            onClick={() => addMorePosts()}
            className="mx-auto bg-social-orange pl-14 pr-14 py-3 rounded-3xl font-semibold text-white text-xs mt-6"
          >
            Laad meer
          </button>
        </section>
